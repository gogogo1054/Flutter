...
class _MyHomePageState extends State<MyHomePage> {
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            ElevatedButton( //  공공데이터 추가
              child: const Text('Covid 공공데이터 파싱',
                style: TextStyle(fontSize: 24)),
              onPressed: () {
                covid19XMLData();
              },
  ...
  void getJsonData() {
    //  안쪽 데이터는 더블쿼테이션으로 처리해야 에러가 생기지 않는다.
    String json = '''
{"items" : {"item" : 
            [{"id" : "1", "name" : "홍길동"},
             {"id" : "2", "name" : "전우치"},
...
''';
    ...
    //  파싱을 원한 key값은 배열 기호를 통해 접근
    List data2 = data1['items']['item'];
    print(data2);

    //  배열의 크기만큼 반복해서 id와 name을 출력
    print('=========================================================================');
...
''';
  ...
  //  확진자 폭증으로 현재 누적 검사자수는 집계되지 않는다.
  print("날짜 - 누적확진자 - 누적사망자");
  //  개선된 for문으로 반복해서 파싱
  for(var item in data2) {
    String createDt = item['createDt'];
    //  파싱된 데이터는 문자열이므로 연산을 위해서는 정수로 변환해줘야함
    //  만약 데이터에 문제가 있는 경우라면 0으로 할당
