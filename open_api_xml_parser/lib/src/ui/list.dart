...
class ListWidget extends StatelessWidget {
  ...
  //  리스트 뷰
  Widget _mkeListView(List<Ev> evs) {
  ...
  //  Provider.of를 통해 데이터에 접근한다. builder만을 업데이트 하기 위해 listen은 false로 한다.
  _evProvider = Provider.of<EvProvider>(context, listen: false);
  _evProvider.loadEvs();  //  EvProvider에 lodaEvs()의 접근
  ...
  return Scaffold(
    ...
    body: Consumer<EvProvider>(builder: (context, provider, widget) {
      //  데이터가 있으면 _makeListView에 데이터를 할당
      if (provider.evs.isNotEmpty) {
        return _mkeListView(provider.evs);
      }
